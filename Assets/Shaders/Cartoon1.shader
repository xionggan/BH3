...
////// Emissive:
                ...
                float3 finalColor = emissive + (((node_7009*node_7331)+(node_7331*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(10,0,_SpecularRange)))*_SpecColor_copy.rgb*_SpecularPower))*_LightColor0.rgb*attenuation*dot(_LightMap_var.rgb,float3(0.3,0.59,0.11)));
...
////// Lighting:
                ...
                float node_7331 = step(_DiffuseSplitValue,max(0,dot(lightDirection,normalDirection)));
                float4 _LightMap_var = tex2D(_LightMap,TRANSFORM_TEX(i.uv0, _LightMap));
                float3 finalColor = (((node_7009*node_7331)+(node_7331*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(10,0,_SpecularRange)))*_SpecColor_copy.rgb*_SpecularPower))*_LightColor0.rgb*attenuation*dot(_LightMap_var.rgb,float3(0.3,0.59,0.11)));
